...
import logoWatermark from "../assets/logo-watermark.png"; // ✅ logo PNG dùng làm watermark

// ✅ Component con: hiển thị sản phẩm tương tự
function SimilarProductItem({ item, logoWatermark, addWatermark }) {
  const [simWatermarked, setSimWatermarked] = React.useState(null);

  React.useEffect(() => {
    if (item.images["Tranh Canvas"]) {
      addWatermark(
        item.images["Tranh Canvas"],
        logoWatermark,
        (dataUrl) => setSimWatermarked(dataUrl)
      );
    }
  ...
  return (
    <Link
      key={item.id}
      to={`/chi-tiet/${item.id}`}
      className="group bg-gray-50 rounded-md shadow hover:shadow-md overflow-hidden transition"
    >
      <div className="relative">
        <img
          src={simWatermarked || item.images["Tranh Canvas"]}
          alt={item.title}
      ...
      <div className="p-2">
        ...
        <p className="text-orange-600 text-sm font-bold">
          {item.price["Tranh Canvas"].toLocaleString()}₫
        </p>
      </div>
    </Link>
  );
...
export default function ChiTietTranh({ addToCart }) {
  ...
  // --- 🖼️ State để lưu ảnh sau khi thêm watermark
  ...
  const addWatermark = (imageSrc, logoSrc, callback) => {
    ...
    img.onload = () => {
      ...
      logo.src = logoSrc;
      logo.onload = () => {
        const logoWidth = img.width * 0.25; // Logo chiếm 25% chiều rộng ảnh
        const logoHeight = (logo.height / logo.width) * logoWidth;
        const x = (img.width - logoWidth) / 2; // Ở giữa ảnh
        const y = (img.height - logoHeight) / 2;

        ctx.globalAlpha = 0.35; // độ trong suốt 35%
  ...
  useEffect(() => {
    if (image) {
      addWatermark(image, logoWatermark, (dataUrl) => setWatermarkedImage(dataUrl));
    }
  }, [image]);
...
const handleBuyNow = () => {
  ...
  // --- Sản phẩm tương tự ---
  const similarItems =
    allArts?.filter((a) => a.category === art?.category && a.id !== art?.id) || [];
      ...
      <div className="max-w-7xl mx-auto bg-white rounded-xl shadow-lg p-6 grid md:grid-cols-2 gap-8">
        ...
        <div>
          ...
          >
            ...
            <div className="absolute bottom-3 left-3 bg-black/40 text-white text-sm px-3 py-1 rounded">
              {selectedType}
            </div>
          </motion.div>
        ...
        <div className="flex flex-col">
          ...
          <div className="flex gap-3 mt-2">
            <button
              ...
              className="flex-1 bg-orange-500 text-white py-3 rounded-md font-semibold hover:bg-orange-600 transition"
            >
              🛒 Thêm vào giỏ
            </button>
            <button
              onClick={handleBuyNow}
              className="flex-1 bg-amber-400 text-white py-3 rounded-md font-semibold hover:bg-amber-500 transition"
            >
              ⚡ Mua ngay
            </button>
          </div>
      ...
      {art.details && (
        <div className="max-w-7xl mx-auto mt-8 bg-gradient-to-br from-white to-gray-50 rounded-2xl shadow-md p-6 border border-gray-100">
          <h2 className="text-2xl font-semibold mb-4 text-gray-800 flex items-center gap-2">
            <span className="text-indigo-600">🖼️</span> Chi tiết sản phẩm
          </h2>
          ...
          >
            ...
            >
              ...
              <img
                src={logoWatermark}
                alt="Watermark"
                className="absolute bottom-4 right-4 w-20 opacity-70 pointer-events-none select-none"
  ...
  <button
    onClick={() => handleAddToCart(art)}  // ✅ Truyền art vào
  ...
  >
    🛒 Giỏ
        ...
        <button
          onClick={handleBuyNow}
          className="flex-1 bg-amber-400 text-white py-3 rounded-md font-semibold"
        >
          ⚡ Mua ngay
        </button>
      </div>
